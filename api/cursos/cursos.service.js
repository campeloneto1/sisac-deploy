...
let CursosService = class CursosService {
    constructor(cursoRepository, logsService, lazyModuleLoader) {
    ...
    async create(object, idUser) {
        var object = this.cursoRepository.create({ ...object, created_by: idUser });
    ...
    async update(id, object, idUser) {
        var data = await this.cursoRepository.findOneBy({ id: id });
        data = { ...object };
